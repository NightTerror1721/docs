...
--- @return any
function SideButton:press(playSound)
```
...
--- @return Basis
function SideButton:getAssociatedBasis(orientation)
```
...
```lua
--- @return Basis
function SideButton:getAssociatedBasis()
```
...
--- @param orientation Orientation
--- @return boolean
function SideButton:isDirectionBlocked(orientation)
```
...
```lua
--- @return Decoration
function SideButton:asDecoration()
```
...
Specific events that `SideButton` can trigger and that are captured with the `OnCustomEvent` hook.
