...
```lua
--- @return any
function SideRotator:invertRotation()
```
...
--- @return Basis
function SideRotator:getAssociatedBasis(orientation)
```
...
```lua
--- @return Basis
function SideRotator:getAssociatedBasis()
```
...
--- @param orientation Orientation
--- @return boolean
function SideRotator:isDirectionBlocked(orientation)
```
...
```lua
--- @return Decoration
function SideRotator:asDecoration()
```
...
Specific events that `SideRotator` can trigger and that are captured with the `OnCustomEvent` hook.
