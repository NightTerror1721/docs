...
--- @return Basis
function SideLaser:getAssociatedBasis(orientation)
```
...
```lua
--- @return Basis
function SideLaser:getAssociatedBasis()
```
...
--- @param orientation Orientation
--- @return boolean
function SideLaser:isDirectionBlocked(orientation)
```
...
```lua
--- @return Decoration
function SideLaser:asDecoration()
```
...
Specific events that `SideLaser` can trigger and that are captured with the `OnCustomEvent` hook.
