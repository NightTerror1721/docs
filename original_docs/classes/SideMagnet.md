...
```lua
--- @return any
function SideMagnet:changeToNextState()
```
...
--- @return Basis
function SideMagnet:getAssociatedBasis(orientation)
```
...
```lua
--- @return Basis
function SideMagnet:getAssociatedBasis()
```
...
--- @param orientation Orientation
--- @return boolean
function SideMagnet:isDirectionBlocked(orientation)
```
...
```lua
--- @return Decoration
function SideMagnet:asDecoration()
```
...
Specific events that `SideMagnet` can trigger and that are captured with the `OnCustomEvent` hook.
